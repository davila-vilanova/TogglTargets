...
protocol ProjectIDsByGoalsProducing {
    ...
    var fetchProjectIDsByGoalsAction: Action<Void, ProjectIDsByGoals.Update, NoError> { get }
}
...
class SQLiteGoalsStore: ProjectIDsByGoalsProducingGoalsStore {
    ...
    /// Keep all existing goals in memory.
    // TODO: if this turns out to be a problem it would be possible to switch to an approach in which
    // an array of ordered goal IDs is kept in memory instead.
    ...
    /// and a project ID as its input, runs in the context defined by the project indexed goals and
    /// the current `ProjectIDsByGoals` value, both prior to the goal addition, update or removal.
    ...
    /// See the note in the `allGoals` property.
